...
if(mysqli_num_rows($result) > 0)
{
	...
	{
		if (strpos($row["fecha_alta"], '/') !== false) {
    $fecha_alta= str_replace("/", "-", $row['fecha_alta'] );
    }else{
		...
		$output .= '
			<tr>
				...
				<td data-label="precio_unitario">'.$row["precio_unitario"].'</td>
				<td data-label="punto_reordenamiento">'.$row["punto_reordenamiento"].'</td>
				<td data-label="fecha_alta">'.$fecha_alta.'</td>
